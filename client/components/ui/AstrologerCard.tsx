...
const AstrologerCard = ({ astrologer }: AstrologerCardProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {  // Fixed type
    e.preventDefault();
    try {
      dispatch(setConnectingAstrologer({
        id: astrologer.userId._id,
        name: astrologer.userId.name,
      }));

      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/chat/init`, {
        method: 'POST',
        headers: {
        ...
        credentials: 'include',
        body: JSON.stringify({
          astrologerId: astrologer.userId._id,
    ...
    <div className="flex flex-col bg-white shadow-sm border border-slate-200 rounded-lg my-6 w-96">
      ...
      <div className="p-3 text-center">
        <div className="flex justify-center mb-2">
          {renderStars(astrologer.ratings?.average || 0)}
        </div>
